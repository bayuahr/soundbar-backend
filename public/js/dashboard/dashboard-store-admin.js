(function (jQuery) {
    ...
    if (document.querySelectorAll('#service-order-by-status').length) {
        const options = {
            series: [{
              name: 'Total',
              data: serviceOrderByStatus.total
            }],
            chart: {
              type: 'bar',
              height: 230,
            ...
            xaxis: {
              categories: serviceOrderByStatus.status,
              labels: {
                minHeight:20,
                maxHeight:20,
                style: {
        ...
        const chart = new ApexCharts(document.querySelector("#service-order-by-status"), options);
        chart.render();
        document.addEventListener('ColorChange', (e) => {
            const newOpt = {colors: [e.detail.detail1, e.detail.detail2],}
            chart.updateOptions(newOpt)
    ...
    if (document.querySelectorAll('#income-monthly-chart').length) {
        const options = {
            series: [{
                name: 'Income',
                data: monthlyIncome
        ...
        const chart = new ApexCharts(document.querySelector("#income-monthly-chart"), options);
        chart.render();
        document.addEventListener('ColorChange', (e) => {
        console.log(e)
        const newOpt = {
